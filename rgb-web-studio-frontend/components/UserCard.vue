...
<div class="user-card group">
	<div class="fill-blue-300 group-hover:fill-accent duration-300 w-2/3 mx-auto">
	...
	<div class="pt-4 flex flex-col justify-between gap-2 basis-0 grow">
		<div>
			Ім'я: {{user.name}} <span class="text-gray-2 text-sm">#{{user.id.substring(user.id.length-6)}}</span>
		</div>
		<div class="basis-0 grow"></div>
		<div class="items-center flex">
			<div class="material-symbols-outlined text-3xl"> picture_as_pdf </div>
			<div class="pl-2">{{user.files?.length}}</div>
		</div>
		<Button1 class="text-white hover:text-black" @click="selectUser" v-if="isSelectButton">
			Обрати
		</Button1>
	</div>
</div>
...
<script setup>
import Button1 from "./Button-1.vue";
...
.user-card:hover{
	box-shadow: 0 0 4px 4px #3572EF inset;
}
</style>