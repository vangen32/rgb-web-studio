<template>
  <div class="w-full">
    <h1 class="title">Деталі файлу ID:{{ file?.id }}</h1>
    <div class="p-6">
...
const fileData = computed(() => {
  ...
  if (file.value) {
    ...
    addField(
      "Дата завантаження",
      file.value.createdAt
        ? DateFormatter.GetDateTime(file.value.createdAt)
        : "-",
    );
    ...
    addField(
      "Розмір, Мб",
      FileSizeConverter.ByteToMb(file.value.size).toFixed(2),
    );
    addField("Власник", file.value.user.name, {
      name: "user-id",
      params: { id: file.value.user.id },
    });
  }
  return fileData;
});
