...
<div class="w-full">
	...
	<div class="p-6">
		<div class="flex flex-wrap gap-4">
			<UserCard v-if="user" :user="user" class="basis-[300px] grow-0 shrink-0"
								:is-select-button="false"/>
			<SelectDropFileArea ref="fileArea" class="group  basis-[300px] grow shrink-0  min-h-[150px]" @send-file="sendFile"/>
		</div>
		<table class="w-full my-6 table-auto border-collapse" v-if="userFilesListData?.files">
			...
			<tbody>
				<tr v-for="file in userFilesListData.files" :key="file.id">
					...
					<td>{{file.format}}</td>
					<td>
						<NuxtLink class="block bg-blue-300 p-1 px-2 rounded-lg hover:bg-blue-400 cursor-pointer" :to="{
							name : 'files-fileId', params : {fileId : file.id}
						}">
							Деталі
						</NuxtLink>
					</td>
		...
		<Pagination v-if="totalPages>1" :pages="totalPages" v-model="currenFilesListPage" class="mx-auto w-fit "/>
	</div>
...
const {data : user, pending : pendingUser, error, refresh : refreshUser} =  useAsyncData(
	async () => {
...
const sendFile = async (file)=>{
	try {
		...
		const res = await $fetch(`${config.public.BASE_URL}/user/file-upload/${route.params.id}`, {
			body : formData,
			method : "POST",
		});
...
watch(user, (newVal)=>{
	if(newVal)
		userStore.setSelectedUser(newVal)
	else
		toUserList();
})
